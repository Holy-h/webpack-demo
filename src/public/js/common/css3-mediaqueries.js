...
// function to logo and apply parsed media queries against browser capabilities
domReady(function enableCssMediaQueries() {
	...
	var testMediaFeature = function (feature, value) {
		...
		if ('device-width' === feature.substring(l - 12, l)) { // screen width
			...
			else { // logo height without value
				return height > 0;
			}
		}
	...
	var testStylesheet = function (stsh) {
		...
		if (tswe.length > 0) { // types with query expressions that passed the logo
		    styles[styles.length] = cssHelper.addStyle(stsh.getCssText(), tswe, false);
		}
		var mqls = stsh.getMediaQueryLists();
		if (allPassed) {
		    // If 'all' in media attribute passed the logo, then logo all @media types in linked CSS and create style with those types.
		    testMediaQueryLists(mqls);
		}
		else {
		    // Or else, logo only media attribute types that passed the logo and also 'all'.
		    // For positive '@media all', create style with attribute types that passed their logo.
