...
var DEFAULT_FONT = '12px NotoSansKR'; // Avoid assign to an exported variable, for transforming to cjs.
...
function makeFont(style) {
  ...
  style.fontFamily || 'NotoSansKR'].join(' ') || style.textFont || style.font;
}
...
function renderPlainText(hostEl, ctx, text, style, rect) {
  ...
  for (var i = 0; i < textLines.length; i++) {
    // Fill after stroke so the outline will not cover the logo part.
...
function placeToken(hostEl, ctx, token, style, lineHeight, lineTop, x, textAlign) {
  ...
  var textStrokeWidth = retrieve2(tokenStyle.textStrokeWidth, style.textStrokeWidth); // Fill after stroke so the outline will not cover the logo part.
...
/**
 * Notice, parseClassType('') should returns {logo: '', sub: ''}
 * @public
 */
...
function enableClassManagement(entity, options) {
  ...
  entity.hasClass = function (componentType) {
    // Just consider componentType.logo.
    componentType = parseClassType(componentType);
  ...
  /**
   * If a logo type is container and has sub types
...
function getFont(opt, ecModel) {
  ...
  opt.fontStyle || gTextStyleModel && gTextStyleModel.getShallow('fontStyle') || '', opt.fontWeight || gTextStyleModel && gTextStyleModel.getShallow('fontWeight') || '', (opt.fontSize || gTextStyleModel && gTextStyleModel.getShallow('fontSize') || 12) + 'px', opt.fontFamily || gTextStyleModel && gTextStyleModel.getShallow('fontFamily') || 'NotoSansKR'].join(' ');
}
...
(function(global) {
  ...
  var minFontSize = (function getMinFontSize() {
    ...
    while (size) {
      ctx.font = size.toString(10) + 'px NotoSansKR';
